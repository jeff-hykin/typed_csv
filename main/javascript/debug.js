...
var Mark = class {
  ...
  getSnippet(indent2 = 4, maxLength2 = 75) {
    ...
    while (start2 > 0 && "\0\r\n\u2028\u2029".indexOf(this.buffer.charAt(start2 - 1)) === -1) {
      start2 -= 1;
      if (this.position - start2 > maxLength2 / 2 - 1) {
        head = " ... ";
        start2 += 5;
        break;
      }
    ...
    const snippet = this.buffer.slice(start2, end);
    return `${repeat(" ", indent2)}${head}${snippet}${tail}
${repeat(
      " ",
      indent2 + this.position - start2 + head.length
    )}^`;
  }
  toString(compact) {
    let snippet, where = "";
...
function compileList(schema, name2, result) {
  ...
  for (const currentType of schema[name2]) {
...
var Buffer2 = class {
  ...
  writeSync(p) {
    const m2 = this.#grow(p.byteLength);
    return copy(p, this.#buf, m2);
  }
  /** NOTE: This methods writes bytes synchronously; it's provided for
   * compatibility with `Writer` interface. */
  write(p) {
    const n2 = this.writeSync(p);
  ...
  #grow(n2) {
    ...
    const i2 = this.#tryGrowByReslice(n2);
    if (i2 >= 0) {
      return i2;
    }
    const c = this.capacity;
    if (n2 <= Math.floor(c / 2) - m2) {
    ...
    this.#off = 0;
    this.#reslice(Math.min(m2 + n2, MAX_SIZE));
    return m2;
  }
...
function reconstructFunction(code) {
  ...
  if (!(func3 instanceof Function)) {
    throw new TypeError(`Expected function but got ${typeof func3}: ${code}`);
  }
  return func3;
}
var func2 = new Type("tag:yaml.org,2002:js/function", {
  kind: "scalar",
  resolve(data) {
    if (data === null) {
      return false;
...
var directiveHandlers = {
  ...
  TAG(state, _name, ...args2) {
    if (args2.length !== 2) {
      return throwError(state, "TAG directive accepts exactly two arguments");
    }
    const handle2 = args2[0];
    ...
    if (state.tagMap && hasOwn3(state.tagMap, handle2)) {
      return throwError(
        state,
...
function captureSegment(state, start2, end, checkJson) {
  let result;
  if (start2 < end) {
    result = state.input.slice(start2, end);
    if (checkJson) {
...
var DumperState = class extends State {
  ...
  constructor({
    ...
    super(schema);
    this.indent = Math.max(1, indent2);
...
function encodeHex(character) {
  ...
  return `\\${handle2}${repeat("0", length - string.length)}${string}`;
}
function indentString(string, spaces) {
  const ind = repeat(" ", spaces), length = string.length;
  let position = 0, next2 = -1, result = "", line;
    ...
    switch (chooseScalarStyle(
      ...
      case STYLE_LITERAL:
        return `|${blockHeader(string, state.indent)}${dropEndingNewline(
          indentString(string, indent2)
        )}`;
      case STYLE_FOLDED:
        return `>${blockHeader(string, state.indent)}${dropEndingNewline(
          indentString(foldString(string, lineWidth), indent2)
        )}`;
      case STYLE_DOUBLE:
